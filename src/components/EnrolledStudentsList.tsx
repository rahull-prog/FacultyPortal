                    ...
                    <table className="w-full border-collapse">
                        ...
                        <tbody>
                            {students.map(student => {
                                ...
                                const attendanceHistory = Array.from({ length: 7 }, () =>
                                    Math.random() > 0.3 ? 'present' : 'absent'
                                );
                                const attendancePercentage = Math.round(
                                    (attendanceHistory.filter(a => a === 'present').length / 7) * 100
                                );
                                        ...
                                        {attendanceHistory.map((status, idx) => (
                                            <td key={idx} className="text-center py-4 px-2">
                                                <div className="flex justify-center">
                                                    <div
                                                        className={`w-7 h-7 rounded flex items-center justify-center ${status === 'present'
                                                            ? 'bg-green-500'
                                                            : 'bg-red-500'
                                                            }`}
